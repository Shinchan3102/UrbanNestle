...
const Input = ({ label, type = "text", disabled = false, required, register, errors, id, hasIcon = false, Icon = IoMailOutline }: Props) => {
    return (
        <div className="w-full relative flex items-center">
            ...
            />
            <label htmlFor={id} className={`absolute cursor-text duration-150 transform -translate-y-5 scale-75 bg-white top-2.5 z-10 peer-placeholder-shown:scale-100  ${hasIcon ? '-translate-x-5' : '-translate-x-3'} peer-placeholder-shown:translate-y-0 peer-placeholder-shown:translate-x-0 peer-focus:scale-75 peer-focus:-translate-y-5 ${hasIcon ? 'peer-focus:-translate-x-5' : 'peer-focus:-translate-x-3'} px-2 ${errors[id] ? 'text-red-500' : 'text-zinc-300 peer-focus:text-black'} ${hasIcon ? 'left-6' : 'left-2'}`}>
                {label}
            </label>
            {
                hasIcon &&
